...
def train(args):
    ...
    if word2id is None and args.data_name in ['weibo', 'resume', 'msra']:  # ner task.
        train_examples = process.load_data('./data/{}/{}.tsv'.format(args.data_name, 'train'))
        train_examples = train_examples[:int(len(train_examples)*args.ratio)]  # For testing.
        eval_examples = process.load_data('./data/{}/{}.tsv'.format(args.data_name, 'dev'))
    else:  # cws task
        all_train_examples = process.load_data('./data/{}/{}.tsv'.format(args.data_name, 'train'))
        np.random.shuffle(all_train_examples)
        all_train_examples = all_train_examples[:int(len(all_train_examples)*args.ratio)]  # For testing.
    ...
    test_examples = process.load_data('./data/{}/{}.tsv'.format(args.data_name, 'test'))
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, start_index in enumerate(tqdm(range(0, len(train_examples), args.train_batch_size))):
            ...
            train_features = process.convert_examples_to_features(batch_examples)
            input_ids, label_ids, input_mask = process.feature2input(device, train_features)
            
            soutput, slogits = smodel(input_ids)

            if smodel.crf is None:
                loss = ce_loss(slogits.view(-1, config.num_labels), label_ids.view(-1))
            else:
            ...
            if tmodel is not None:
                ...
                if not args.distill_hidden:
                    loss = (1 - args.alpha) * loss + args.alpha * mse_loss(w_wei * slogits, w_wei * tlogits) # targets
                else:
                    loss = (1 - args.alpha) * loss + args.alpha * mse_loss(w_wei * soutput, w_wei * toutput)  # hiddens
    ...
    for start_index in range(0, len(eval_examples), eval_batch_size):
        ...
        for label_id, label_pred, input_mas in zip(label_ids, label_preds, input_mask):
            ...
            for iid, pid, imas in zip(label_id, label_pred, input_mas):
                ...
                tmpp.append(config.id2label.get(pid, config.num_labels))
            if len(tmpt) < 2:
                continue
            y_true.append(tmpt[1:-1])
            y_pred.append(tmpp[1:-1])
